...
class PirateBaySearch(BaseSearch):
    ...
    def get_search_url(self, name, maskval):
        return "https://theproxypirate.pw/search/"+"+".join(name.split(" ")).replace("'","")+"+"+"+".join(maskval.split(" ")) + "/0/7/0"
    ...
    def validate_page(self, tor):
        ...
        turl = tor.replace("https://torrents.theproxypirate.pw", "https://theproxypirate.pw/torrent")
...
class KATSearch(BaseSearch):
    ...
    def get_search_url(self, name, maskval):
        return "https://k.unblocked.pw/usearch/" + " ".join(name.split(" ")).replace("'","")+" "+" ".join(maskval.split(" ")) + "/"
    ...
    def get_torrent_from_validated_page(self, page):
        ...
        return "http://k.unblocked.pw/" + page.replace("torrent_", "")
...
class SpiderBro:
    ...
    def get_episode_list(self, series_name):
        ...
        try:
            ...
            show = result[0]
            highest_season = len(show)
            # pytvdb object isn't uniform, sometimes show[0] can be season 1 or season 0 (specials), so len(show) isn't always a reliable way to get the no of seasons
            try:
                if(show[0].season_number == 0):
                    highest_season = len(show)-1
            except error.TVDBIndexError:
                pass
            for s in range(1, highest_season+1):
                season_no = show[s].season_number
                season = show[season_no]
                for ep in range(len(season)+1):
                    if(ep != 0):
                    # get airdate, ignore if > today
                        if (season[ep].FirstAired != ''):
                            airdate = season[ep].FirstAired
                            # need to compare current date to air date, ignore if not aired yet
                            if date.today() > airdate:
                                aired_list.append((str(season_no), str(ep)))
        except Exception, e:
            self.logger.error("\tCould not get episode list from thetvdb")
            self.logger.error(e)
            self.logger.error("")
    ...
    def hunt_eps(self, series_name):
        ...
        if ended and not ep_list:
            self.logger.info("\tGot all episodes for this, skipping in future (NOTE: FEATURE CURRENTLY DISABLED)")
